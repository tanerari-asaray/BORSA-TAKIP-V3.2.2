...
class OpportunityAdapter(private val onClick:(Opportunity)->Unit={}) : ListAdapter<Opportunity,OpportunityAdapter.Holder>(DiffCallback){
 ...
 class Holder(private val binding:ItemOpportunityBinding,private val onClick:(Opportunity)->Unit):RecyclerView.ViewHolder(binding.root){
  fun bind(item:Opportunity){
   ...
   binding.companyNameText.text=item.companyName?.takeIf{it.isNotBlank()} ?: "BIST • ${item.symbol}"
   binding.scoreText.text="${item.score}/100"
   binding.signalText.text="${item.direction} • ${item.confidence}"
   binding.signalText.setTextColor(ContextCompat.getColor(binding.root.context,when(item.direction){"LONG"->R.color.green;"SHORT"->R.color.red;else->R.color.yellow}))
   binding.priceText.text="Fiyat ${fmt(item.price)} TL"
   binding.buyText.text=item.entry?.let{"Giriş ${fmt(it)} TL"}?:"Giriş: Veri yetersiz"
   binding.targetText.text=item.target1?.let{t1->item.target2?.let{t2->"Hedef ${fmt(t1)} / ${fmt(t2)} TL"}?:"Hedef ${fmt(t1)} TL"}?:"Hedef: Veri yetersiz"
   binding.stopText.text=item.stop?.let{"Stop ${fmt(it)} TL"}?:"Stop: Veri yetersiz"
   binding.bedelsizText.text=item.riskReward?.let{"Risk/Getiri 1:${fmt(it)}"}?:"Risk/Getiri: Veri yetersiz"
  }
