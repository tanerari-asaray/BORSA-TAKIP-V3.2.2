...
/** V3.2.6 FIRSAT KONTROLÜ - seçici teknik fırsat motoru. */
class ScoringEngine {
    ...
    private fun evaluateSide(stock: Stock, t: tr.borsatakip.v3.model.TechnicalSnapshot, isLong: Boolean): SideEval {
        ...
        fun side(long: Boolean, short: Boolean) = if (isLong) long else short

        val dataPass = !stock.isDemo && candles.size >= 50 && stock.dataTimestamp > 0L && candles.takeLast(20).all { c -> c.open > 0 && c.high > 0 && c.low > 0 && c.close > 0 && c.high >= c.low && c.volume.isFinite() }
        ...
        val ema20 = t.ema20; val ema50 = t.ema50; val ema200 = t.ema200
        val prevEma20 = previous.ema20; val prevEma50 = previous.ema50
        val slopePass = side(ema20 != null && prevEma20 != null && ema50 != null && prevEma50 != null && ema20 >= prevEma20 && ema50 >= prevEma50, ema20 != null && prevEma20 != null && ema50 != null && prevEma50 != null && ema20 <= prevEma20 && ema50 <= prevEma50)
        val trendPass = side(ema20 != null && ema50 != null && price > ema20 && price > ema50 && ema20 > ema50 && slopePass, ema20 != null && ema50 != null && price < ema20 && price < ema50 && ema20 < ema50 && slopePass)
        ...
        val closeQuality = side(closePos >= 0.65, closePos <= 0.35)
        ...
        val preferredMomentum = side(rsi != null && rsi in 55.0..68.0 && macdLong, rsi != null && rsi in 32.0..45.0 && macdShort)
        val acceptableMomentum = side(rsi != null && rsi > 50 && macdLong, rsi != null && rsi < 50 && macdShort)

        val atr = t.atr14
        val distanceAtr = if (atr != null && atr > 0 && t.ema20 != null) abs(price - t.ema20) / atr else null
        val rsiExtended = side((rsi ?: 50.0) >= 80, (rsi ?: 50.0) <= 20)
        ...
        val recentLow = prev20.minOfOrNull { it.low }; val recentHigh = prev20.maxOfOrNull { it.high }
        val stop = if (atr != null && atr > 0) if (isLong) {
            val a = price - 1.5 * atr; val s = recentLow?.minus(0.10 * atr); listOfNotNull(a.takeIf { it > 0 }, s?.takeIf { it > 0 && it < price }).maxOrNull()
        } else {
            val a = price + 1.5 * atr; val s = recentHigh?.plus(0.10 * atr); listOfNotNull(a, s?.takeIf { it > price }).minOrNull()
        } else null
        ...
        val rr = if (risk != null && risk > 0 && target1 != null) { val reward = if (isLong) target1 - price else price - target1; if (reward > 0) reward / risk else null } else null
        ...
        val regimePass = when { regime == null || regime == "NEUTRAL" -> true; isLong && regime == "BULL" -> true; !isLong && regime == "BEAR" -> true; else -> (rsDirectional ?: Double.NEGATIVE_INFINITY) >= 4.0 }

        val trendPoints = when { trendPass && ema200 != null && side(price > ema200, price < ema200) -> 20; trendPass -> 16; side(ema20 != null && price > ema20, ema20 != null && price < ema20) -> 8; else -> 2 }
        val momentumPoints = when { preferredMomentum -> 15; acceptableMomentum -> 11; side(rsi != null && rsi > 50, rsi != null && rsi < 50) -> 7; else -> 2 }
        val volumePoints = when { volumeRatio == null -> 0; volumeRatio >= 2 -> 15; volumeRatio >= 1.5 -> 13; volumeRatio >= 1.2 -> 10; volumeRatio >= 1 -> 7; else -> 2 }
        val structurePoints = when { confirmedBreak -> 15; structurePass -> 11; closeQuality -> 6; else -> 2 }
        val rsPoints = when { rsDirectional == null -> 0; rsDirectional >= 8 -> 15; rsDirectional >= 4 -> 12; rsDirectional >= 2 -> 9; rsDirectional >= 0 -> 6; else -> 1 }
        val regimePoints = when { regime == null -> 0; regime == "NEUTRAL" -> 6; isLong && regime == "BULL" -> 10; !isLong && regime == "BEAR" -> 10; regimePass -> 4; else -> 0 }
        val rrPoints = when { rr == null -> 0; rr >= 2.5 -> 10; rr >= 2 -> 9; rr >= 1.75 -> 7; rr >= 1.25 -> 3; else -> 0 }
        val components = listOf(ScoreComponent("Trend", trendPoints, 20), ScoreComponent("Momentum", momentumPoints, 15), ScoreComponent("Hacim", volumePoints, 15), ScoreComponent("Kırılım / Yapı", structurePoints, 15), ScoreComponent("Göreceli Güç", rsPoints, 15), ScoreComponent("Piyasa Rejimi", regimePoints, 10), ScoreComponent("Risk / Getiri", rrPoints, 10))

        return SideEval(components.sumOf { it.points }.coerceIn(0, 100), components, dataPass, liquidityPass, trendPass, structurePass, volumePass, rrPass, overextended, noCriticalLevel, regimePass, stop, target1, target2, rr)
    }

    private fun classify(score: Int, gatesPass: Boolean) = when { !gatesPass -> "UYGUN DEĞİL"; score >= 90 -> "A+ TEKNİK UYUM"; score >= 85 -> "GÜÇLÜ FIRSAT"; score >= 80 -> "FIRSAT ADAYI"; score >= 70 -> "İZLE"; else -> "ZAYIF" }
}
