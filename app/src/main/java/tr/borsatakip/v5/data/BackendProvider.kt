    ...
    suspend fun loadViop(): Result<List<ViopContract>> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            try {
                ...
                val a = JSONObject(con.inputStream.bufferedReader().use { it.readText() }).optJSONArray("items")
                    ?: return@runCatching emptyList()
                (0 until a.length()).mapNotNull { i ->
                    val x = a.optJSONObject(i) ?: return@mapNotNull null
                    val symbol = x.optString("symbol").takeIf { it.isNotBlank() } ?: return@mapNotNull null
                    ViopContract(
                        symbol,
                        x.optString("underlying"),
                        x.optString("expiry"),
                        x.optDouble("lastPrice", Double.NaN),
                        x.optDouble("dailyChangePct", 0.0),
                        x.optDouble("tickSize", Double.NaN).takeIf { !it.isNaN() },
                        x.optDouble("multiplier", Double.NaN).takeIf { !it.isNaN() },
                        x.optLong("openInterest").takeIf { it > 0 },
                        x.optDouble("volume", Double.NaN).takeIf { !it.isNaN() },
