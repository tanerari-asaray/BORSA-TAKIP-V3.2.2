...
input.hint="İlk harfi yazın (örn. T)"
...
input.setOnItemClickListener{_,_,position,_->input.setText(input.adapter.getItem(position).toString());input.setSelection(input.text.length)}
val dialog=android.app.AlertDialog.Builder(this).setTitle("Takip Listesine Hisse Ekle").setMessage("İlk harfi yazdığınız anda BIST hisseleri filtrelenir. Bir hisseyi seçip EKLE'ye basın.").setView(input).setNegativeButton("İPTAL",null).setPositiveButton("EKLE"){_,_->val symbol=input.text.toString().trim().uppercase(Locale("tr","TR"));if(symbol.isBlank())return@setPositiveButton;val prefs=getSharedPreferences("watchlist",MODE_PRIVATE);val set=prefs.getStringSet("symbols",emptySet())?.toMutableSet()?:mutableSetOf();set.add(symbol);prefs.edit().putStringSet("symbols",set).apply();renderWatchlist()}.create()
dialog.setOnShowListener{input.requestFocus();dialog.window?.setSoftInputMode(android.view.WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE)}
dialog.show()
}
'''
